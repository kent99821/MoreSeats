      ...
      if (event.flag === 0) {
        ...
        switch (event.flag) {
          ...
          case 6:
            ...
            case 8:
              // 检查是否有人在学习
              PageData.findr = await db.collection('rooms').where({
                roomId: event.roomId
              }).field({
                "chairs.infos": true,
              }).get()
              PageData.chairsInfo = PageData.findr.data[0].chairs.infos
              //  对每一个位置进行签退
              for (let i = 0; i < PageData.chairsInfo.length; i++) {
                if (PageData.chairsInfo[i].state !== false) {
                  PageData.resCode = 203
                  return {
